...
function hours_add () {
	...
	$sql  = "SELECT u.userid, CONCAT(first, ' ', last) as name FROM users u, usergroups ug WHERE";
...
function hours_edit ($hid) {
	GLOBAL $db, $user_name;
	$sql .= "SELECT h.*, CONCAT(first, ' ', last) as name FROM hours h, users u WHERE h.userid = u.userid AND h.id = {$hid} LIMIT 1";
	$result = mysql_query($sql, $db);
...
function hours_del ($hid) {
	GLOBAL $db, $user_name;
	$sql .= "SELECT h.*, CONCAT(first, ' ', last) as name, showname FROM hours h, users u, shows s WHERE h.userid = u.userid AND h.showid = s.showid AND h.id = {$hid} LIMIT 1";
	$result = mysql_query($sql, $db);
	$recd = mysql_fetch_array($result);
...
function hours_add_do() {
	GLOBAL $db;
	$sql = "INSERT INTO hours ( userid, showid, date, worked ) VALUES ( '{$_REQUEST['userid']}' , '{$_REQUEST['showid']}' , '{$_REQUEST['date']}' , '{$_REQUEST['worked']}' )";
	$result = mysql_query($sql, $db);
	thrower("Hours Added");
}
...
function hours_edit_do($id) {
	GLOBAL $db;
	$sql = "UPDATE hours SET showid = '{$_REQUEST['showid']}', date = '{$_REQUEST['date']}', worked = '{$_REQUEST['worked']}' WHERE id = '{$id}'";
	$result = mysql_query($sql, $db);
...
function hours_view_pick() {
	...
	$html .= "<div id=\"genform\"><form method=\"post\" action=\"view-hours\" name=\"form1\">\n";
        $html .= "<div class=\"frmele\">Employee: <select name=\"userid\" style=\"width: 25em\" >\n";
        $sql  = "SELECT userid, CONCAT(first, ' ', last) as name FROM users WHERE";
        $sql .= perms_isemp($user_name) ? " username = '{$user_name}' AND" : "";
...
function hours_view($userid) {
	...
	$sql  = "SELECT CONCAT(first, ' ', last) as name, worked, date, showname, h.id as hid FROM users u, shows s, hours h WHERE ";
