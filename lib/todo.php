...
class tdtrac_todo {
	...
	private function add_form() {
		...
		$form = new tdform(array(
			'action' => "{$TDTRAC_SITE}json/save/base:todo/id:0/",
			'id' => 'todo_add_form'
		));
		...
		$result = $form->addDrop(array(
			...
			'label' => 'Assigned To',
			'options' => array_merge(
				array(array('0', '-unassigned-')),
				db_list(get_sql_const('todo'), array('userid', 'name'))
			)
		));
	...
	private function view($condition = null, $type = 'user') {
		...
		if ( is_null($condition) ) {
			...
			$list->setFormat(
				"<a href='{$TDTRAC_SITE}%s'><h3>%s</h3><span class='ui-li-count'>%d</span></a>"
			);
			
			$sql = "SELECT u.userid, CONCAT(first, ' ', last) as name, count(t.id) as num" .
				" FROM {$MYSQL_PREFIX}users u" .
			...
			$sql = "SELECT t.showid FROM {$MYSQL_PREFIX}todo t, {$MYSQL_PREFIX}shows s" .
			...
			if ( mysql_num_rows($result) > 0 ) {
				while ( $row = mysql_fetch_array($result) ) {
					...
					$list->addRow(array(
						"todo/view/type:show/id:{$row['showid']}/",
						$row['showname'],
						$the_num
					));
				}
			...
			$todo_num = get_single(
				"SELECT COUNT(*) as num FROM {$MYSQL_PREFIX}todo" .
				" WHERE assigned = {$this->user->id} AND complete = 0"
			);
			$odue_num = get_single(
				"SELECT COUNT(*) as num FROM {$MYSQL_PREFIX}todo" .
				" WHERE complete = 0 AND due < NOW()"
			);
			
			$list->addDivide("Other Options");
			$list->addRow(array(
				'todo/view/id:1/type:overdue/',
				'Overdue Items',
				$odue_num
			));
			$list->addRow(array(
				"todo/view/id:{$this->user->id}/type:user/",
				'Your Personal List',
				$todo_num
			));
			...
			$list->setFormat(
				"<a class='todo-menu' data-done='0' data-recid='%d' data-edit='%d' href='#'>" .
				"<h3>%s</h3><p>" .
			...
			if ( $type == 'user' ) {
				...
				$list->addRaw("<li data-theme='b' id='todo-list-header'><h3>".$this->user->get_name($thiscond)."'s Todo List</h3> <span class='ui-li-count'>{$num}</span></li>");
			} elseif ( $type =='show' ) {
				...
				$list->addRaw("<li data-theme='b' id='todo-list-header'><h3>{$showname[0]}'s Todo List</h3> <span class='ui-li-count'>{$num}</span></li>");
			} elseif ( $type == 'overdue' ) {
			...
			if ( mysql_num_rows($result) < 1 ) {
				...
				while ( $row = mysql_fetch_array($result) ) {
					...
					$assig = ( $type == 'user' ) ?
						$row['showname'] :
						( ( $row['assigned'] > 0 ) ?
							$this->user->get_name($row['assigned']) :
							"-unassigned-"
						);
						
					$statu = ( ( $row['complete'] == 1 ) ?
						'done' :
						"Due: {$row['duedate']}"
					);
					
					$editable = ( $this->user->can('edittodo') ) ? 1 : 0;
			...
			return array_merge(
				$list->output(),
				array("<br /><br /><a class=\"ajax-email\" data-email='{\"action\": \"todo\", \"id\": \"{$thiscond}\", \"type\": \"{$type}\"}' data-role=\"button\" data-theme=\"d\" href=\"#\">E-Mail this Report to Yourself</a>")
			);
		}
	}
